        ...
        # to get positions correct
            ...
            for i, j in zip(samples, range(len(samples))):
                with open(args.filename + "." + str(l) + ".rep{}.window{}.txt".format(k, j), "w") as f:
                    ...
                    f.write(str(i[1]))
                print("sample taken")
                name1=args.filename + "." + str(l) + ".rep{}.window{}.txt".format(k, j)
                name2=args.filename + "." + str(l) + ".rep{}.window{}.sh".format(k, j)
                print(name1)
                print(name2)
                with open(name2, "w") as f1:
                    f1.write("#!/bin/bash\n")
                    f1.write("python /bigraid/khoih/diploSHIC/diploSHIC.py fvecSim haploid"+" /bigraid/khoih/lowmutrate/"+name1+" /bigraid/khoih/lowmutrate/"+name1+".haploid.fvec --numSubWins 11\n")
                print("bash generated")
                os.chmod("/bigraid/khoih/lowmutrate/"+name2, 0o755)
                subprocess.call("/bigraid/khoih/lowmutrate/"+name2)
                os.remove("/bigraid/khoih/lowmutrate/"+name1)
                os.remove("/bigraid/khoih/lowmutrate/"+name2)
