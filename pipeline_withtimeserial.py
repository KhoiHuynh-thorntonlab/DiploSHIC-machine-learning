...
class Traj(object):
    ...
    def __init__(self, timepoints, rng, pop, repargs):
    ...
    def take_sample(self, rng, pop):
            s = self.pop.sample(rng=self.rng, nsam=25,seperate=False)
    ...
    def trajectory(self, pop):
        ...
        # population data.
            ma = np.array(self.pop.mutations.array())
    ...
    def __call__(self, pop):
        ...
        if self.timepoints >10*pop.N:
                    self.take_sample(rng, pop)
                    self.trajectory(pop)
                    with lzma.open(self.filename +self.timepoints + '.lzma', "ab") as f:
                        print("pickling")
                        pickle.dump((repid, pop), f, -1)
                        print("done pickling")
                    print("writing samples")
                    for i, j in zip(samples, range(len(samples))):
                        with open(self.filename +self.timepoints + ".rep{}.window{}.txt".format(repid, j), "w") as f:
...
def runsim(repargs):
    ...
    sampler = Traj(timepoints, rng, pop, repargs)
    print(sampler)
